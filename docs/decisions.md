...
- README "Usage" §3 shape updated: `--proof`/`--public_inputs` are **raw byte strings** (hex, no `0x`), not file paths. The contract's `verify_and_transfer(proof: BytesN<256>, public_inputs: Bytes, ...)` args are `Bytes`/`BytesN`, which the stellar CLI renders as `<256_hex_bytes>`/`<hex_bytes>`; passing `./proof.json` (a filename) was never valid. The prover now emits `proof.blob.json` / `public_inputs.blob.json` holding those byte strings so the README command works verbatim. (Per the Day 4 brief, README was updated to match reality and the reason is logged here.)
